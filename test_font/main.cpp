...
uint32_t swap_endian(uint32_t a) {
	uint8_t temp[4];
...
uint16_t swap_endian(uint16_t a) {
	uint8_t temp[2];
	uint8_t* bytes = reinterpret_cast<uint8_t*>(&a);
	temp[0] = bytes[1];
	temp[1] = bytes[0];
...
void swap_endian_inplace(uint32_t& a) {
	uint8_t temp[4];
	uint8_t* bytes = reinterpret_cast<uint8_t*>(&a);
	temp[0] = bytes[3];
	temp[1] = bytes[2];
...
void swap_endian_inplace(uint16_t& a) {
	uint8_t temp[2];
	uint8_t* bytes = reinterpret_cast<uint8_t*>(&a);
	temp[0] = bytes[1];
	temp[1] = bytes[0];
...
int main() {
	...
	for (auto i = 0u; i < num_tables; ++i) {
		...
		const char null_terminated_tag[5]
			= { my_table.tag[0], my_table.tag[1], my_table.tag[2], my_table.tag[3], '\0' };
...
void read_format_0(std::ifstream& f) {
...
void read_format_2(std::ifstream& f) {
	...
	// uint8_t num_sub_headers = (length - (3u + 256u) * sizeof(uint16_t)) / (5u * sizeof(uint16_t));
...
void read_format_4(std::ifstream& f) {
	...
	const uint16_t variable = (length - (8 + 4 * seg_count) * sizeof(uint16_t)) / sizeof(uint16_t);
	...
	for (auto i = 0u; i < seg_count; ++i) swap_endian_inplace(end_code[i]);
	swap_endian_inplace(reserved_pad);
	for (auto i = 0u; i < seg_count; ++i) swap_endian_inplace(start_code[i]);
	for (auto i = 0u; i < seg_count; ++i) swap_endian_inplace(id_delta[i]);
	for (auto i = 0u; i < seg_count + variable; ++i) swap_endian_inplace(iro_gia[i]);
	...
	for (int32_t unicode = 0u; unicode < UINT16_MAX; ++unicode) {
		...
		printf("id_range_offset[%u] = %u\n", i, iro_gia[i]);
		printf("id_delta[%u] = %u\n", i, id_delta[i]);
		if (iro_gia[i] != 0) {
			// uint16_t* glyph_index_address = id_range_offset[i] + 2u * (unicode - start_code[i]) + &id_range_offset[i];
			// size_t glyph_index = *(&id_range_offset[i] + id_range_offset[i] / 2u + (unicode - start_code[i]));
			glyph_index = *(&iro_gia[i] + iro_gia[i] / 2u + (unicode - start_code[i]));

			printf("glyph index : %zu\n", glyph_index);
			if (glyph_index > 0) {
				glyph_id = (glyph_index + id_delta[i]) % 65536;
			}
			else {
			...
			printf("&id_range_offset[%u] = %u\n", i, &iro_gia[i]);
			printf("unicode - start_code[%u] = %i - %u = %i\n", i, unicode, start_code[i], unicode - start_code[i]);
		}
		else {
			glyph_id = id_delta[i] + unicode;
			printf("iro_gia val %u\n", iro_gia[seg_count + glyph_id]);
		}
		...
		// unicode_to_glyph.push_back(glyph_index_array[glyph_index]);
		unicode_to_glyph.push_back(1ui16);
	}
